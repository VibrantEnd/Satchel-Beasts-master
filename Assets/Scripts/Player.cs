...
public class Player : MonoBehaviour
{
    ...
    {
        Movement();
        if (Input.GetKeyDown(KeyCode.G) && attackAnimReduce <= 0)
        {
    ...
    void Movement()
    {
        float moveX = Input.GetAxisRaw("P1Horiz");
        float moveY = Input.GetAxisRaw("P1Vert");
        ...
        input.x = Input.GetAxisRaw("P1Horiz");
        input.y = Input.GetAxisRaw("P1Vert");
        input.Normalize();
    }
    void Animate()
    {
    ...
    void Attack()
    {
        if (Cooldown <= 0)
        {
            ...
            instProjectile.GetComponent<Projectile>().damage = Damage;
